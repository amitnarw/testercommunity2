...
export interface HubSubmittedAppResponse {
  ...
  totalDay: number;
  /**
   * Handshake owner daily gate: when `locked`, the joined-testers list is
   * only viewable after the owner submits today's proof on their reciprocal
   * relations. Populated by getSingleHubAppDetails (owner view, HANDSHAKE,
   * TESTING_ACTIVE); null everywhere else (no gating).
   */
  ownerDailyGate?: {
    locked: boolean;
    required: number;
  ...
  testerRelations: {
    ...
    /**
     * The tester's own campaign they offered in exchange (HANDSHAKE).
     * Populated by getSingleHubAppDetails owner enrichment.
     */
    offeredApp?: {
      id: number;
      androidApp: { appName: string; appLogoUrl: string | null } | null;
    } | null;
    dailyVerifications?: {
      id: number;
      dayNumber: number;
