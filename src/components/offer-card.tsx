...
export function OfferCard({ isCollapsed }: OfferCardProps) {
  ...
  return (
    <Card
      data-loc="OfferCard"
      className="rounded-xl overflow-hidden bg-gradient-to-br from-primary to-accent text-primary-foreground border-none"
    >
      <CardContent className="p-4 text-center">
        ...
        <p className="text-xs text-primary-foreground/80 mb-3">
          Check out our new point packages.
        </p>
        <Button size="sm" variant="secondary" className="w-full h-8" asChild>
          <Link href="/pricing">
            Buy Points <ArrowRight className="ml-1 h-4 w-4" />
          </Link>
        </Button>
      </CardContent>
    </Card>
