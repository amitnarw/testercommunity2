...
export default function AppTestingOngoingPage({
  ...
  const {
    ...
    isRefetching: appDetailsIsRefetching,
  ...
  const handleDayClick = (day: number) => {
    // 1. Find the user's tester relation (Assuming first meaningful one or hack to get 'my' relation)
    // In a real app, useAuth() -> find relation where testerId === auth.user.id
    const relation = appDetails?.testerRelations?.[0]; // Fallback to first

    // 2. Find verification for this day
    const verification = relation?.dailyVerifications?.find(
      (v) => v.dayNumber === day,
    );

    if (verification) {
      // Cast the string verification to the stricter type if needed or map it
      // Prisma returns Date strings mostly over JSON
      setSelectedVerification({
        id: verification.id,
        dayNumber: verification.dayNumber,
        proofImageUrl: verification.proofImageUrl,
        status: verification.status,
        verifiedAt: verification.verifiedAt, // Ensure string
        rejectionReason: verification.rejectionReason || undefined,
        metaData: verification.metaData,
      });
      setIsModalOpen(true);
    } else {
      // Mock for demo if data missing (e.g. older days)
          ...
          {appDetailsIsPending ? (
            <>
              <div className="flex flex-col gap-10 lg:col-span-2">
                ...
                {/* Developer Instructions Skeleton */}
                ...
                <div className="relative">
                  ...
                  >
                    <SubmittedFeedback
                      ...
                      hubId={id}
                      refetch={appDetailsRefetch}
                      isLoading={appDetailsIsRefetching}
                    />
                  </div>
                  {isTestingNotStarted && (
                    <div className="absolute inset-0 flex items-center justify-center">
